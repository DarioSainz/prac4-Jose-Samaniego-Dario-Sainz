...
Image<unsigned char> read_png(const std::string &filename) {
  ...
  if(!png) { fclose(fp); abort(); }

  png_infop info = png_create_info_struct(png);
  if(!info) { png_destroy_read_struct(&png, NULL, NULL); fclose(fp); abort(); }
  ...
  if(color_type == PNG_COLOR_TYPE_GRAY_ALPHA) png_set_gray_to_rgb(png);
  ...
  if(color_type == PNG_COLOR_TYPE_RGB) channels = 3;
    ...
    std::cerr << "Unsupported color type " << static_cast<int>(color_type) << std::endl;
    png_destroy_read_struct(&png, &info, NULL);
    fclose(fp);
  ...
  for(int i = 0; i < image.height; ++i) delete [] rows[i];
  delete [] rows;
...
void write_png(const std::string &filename, const Image<unsigned char> &image) {
  ...
  png_infop info = png_create_info_struct(png);
  if (!info) { png_destroy_write_struct(&png, NULL); fclose(fp); abort(); }
...
Image<unsigned char> read_jpeg(const std::string &filename) {
    ...
    if(fHandle == NULL) return Image<unsigned char>();
    ...
    const unsigned long imgWidth = info.output_width;
    const unsigned long imgHeight = info.output_height;
    const int numComponents = info.output_components;

    Image<unsigned char> image(imgWidth, imgHeight, numComponents);
    while(info.output_scanline < info.output_height) {
        lpRowBuffer[0] = &image.matrix[numComponents * info.output_width * info.output_scanline];
...
int write_jpeg(const std::string &filename, const Image<unsigned char> &image, int quality=90) {
    ...
    info.in_color_space = (image.channels == 3) ? JCS_RGB : JCS_GRAYSCALE;
